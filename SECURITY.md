...
2. **GitHub**: Use [GitHub Security Advisories](https://github.com/beykuet/MyMolt/security/advisories/new)
...
MyMolt implements defense-in-depth security:
...
MyMolt Docker images follow CIS Docker Benchmark best practices:
...
docker build -t mymolt .
docker inspect --format='{{.Config.User}}' mymolt
# Expected: 65534:65534

# Run with read-only filesystem (production hardening)
docker run --read-only -v /path/to/workspace:/workspace mymolt gateway
```
