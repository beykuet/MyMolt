...
```bash
# Check config
cat ~/.mymolt/config.toml | grep -A 5 telegram
...
| Health check | <5s | `time mymolt channel doctor` |
...
5. **Monitor logs** for any issues:
   ...
   mymolt daemon
   # or
   RUST_LOG=info mymolt channel start
   ```
...
- Issues: https://github.com/beykuet/MyMolt/issues
