...
const OPEN_SKILLS_SYNC_MARKER: &str = ".mymolt-open-skills-sync";
const OPEN_SKILLS_SYNC_INTERVAL_SECS: u64 = 60 * 60 * 24 * 7;

/// A skill is a user-defined or community-built capability.
/// Skills live in `~/.mymolt/workspace/skills/<name>/SKILL.md`
...
fn open_skills_enabled() -> bool {
    if let Ok(raw) = std::env::var("MYMOLT_OPEN_SKILLS_ENABLED") {
...
fn resolve_open_skills_dir() -> Option<PathBuf> {
    if let Ok(path) = std::env::var("MYMOLT_OPEN_SKILLS_DIR") {
    ...
    if !readme.exists() {
        std::fs::write(
            ...
            "# MyMolt Skills\n\n\
             ...
             ```bash\n\
             mymolt skills install <github-url>\n\
             mymolt skills list\n\
             ```\n",
        )?;
    }
    ...
    match command {
        crate::SkillCommands::List => {
            ...
            if skills.is_empty() {
                ...
                println!();
                println!("  Create one: mkdir -p ~/.mymolt/workspace/skills/my-skill");
                println!("              echo '# My Skill' > ~/.mymolt/workspace/skills/my-skill/SKILL.md");
                println!();
                println!("  Or install: mymolt skills install <github-url>");
            } else {
...
"#,
    ...
    #[test]
    fn skills_dir_path() {
        let base = std::path::Path::new("/home/user/.mymolt");
        let dir = skills_dir(base);
        assert_eq!(dir, PathBuf::from("/home/user/.mymolt/skills"));
    }
