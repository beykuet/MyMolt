#!/bin/bash
# MyMolt Telegram Integration Test Suite
...
if [ -f "target/release/mymolt" ]; then
    BINARY_SIZE=$(ls -lh target/release/mymolt | awk '{print $5}')
    SIZE_BYTES=$(stat -f%z target/release/mymolt 2>/dev/null || stat -c%s target/release/mymolt)
...
CONFIG_PATH="$HOME/.mymolt/config.toml"
if [ -f "$CONFIG_PATH" ]; then
    ...
    if grep -q "\[channels_config.telegram\]" "$CONFIG_PATH"; then
        ...
        warn "Telegram not configured - run 'mymolt onboard' first"
    fi
else
    warn "No config file found - run 'mymolt onboard' first"
fi
...
START_TIME=$(date +%s)
HEALTH_OUTPUT=$(timeout 10 target/release/mymolt channel doctor 2>&1 || true)
END_TIME=$(date +%s)
