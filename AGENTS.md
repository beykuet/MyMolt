# AGENTS.md — MyMolt Agent Engineering Protocol
...
MyMolt is a Rust-first autonomous agent runtime optimized for:
...
- If identity-like naming is required in tests/examples, use MyMolt-native labels only (`MyMoltAgent`, `mymolt_user`, `mymolt_node`).
...
- If identity-like context is unavoidable, use MyMolt-scoped roles/labels only (for example: `MyMoltAgent`, `MyMoltOperator`, `mymolt_user`) and avoid real-world personas.
- Recommended identity-safe naming palette (use when identity-like context is required):
    - actor labels: `MyMoltAgent`, `MyMoltOperator`, `MyMoltMaintainer`, `mymolt_user`
    - service/runtime labels: `mymolt_bot`, `mymolt_service`, `mymolt_runtime`, `mymolt_node`
    - environment labels: `mymolt_project`, `mymolt_workspace`, `mymolt_channel`
